...
import CurrentUserRolesContext from "./CurrentUserRolesContext";

export default function Navbar() {
  const [cookies, setCookie, removeCookie] = useCookies(['userId']);
  const rolesContext = useContext(CurrentUserRolesContext);
  ...
  return (
    <nav className="w-full flex items-center justify-between p-4 shadow-md rounded-[15px] bg-red-50">
      ...
      <div className="flex items-center gap-4">
        ...
        {userRoles.some(role => role.name === 'admin') && pathname !== "/admin" && (
          <Link href="/admin">
            <div
              className={`${
